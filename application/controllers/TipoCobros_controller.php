...
class 	TipoCobros_controller extends CI_Controller {
//constructor
	public function __construct(){
		...
		$this->load->model("TipoCobros_model");
...
//carga una vista llamada list
	...
	{
		$data = array(
			'tipocobros' => $this->TipoCobros_model->getTipoCobros(),
		...
		$this->load->view('tipocobros/list',$data);
		$this->load->view('plantilla/footer_plugins');
		$this->load->view('tipocobros/script_tipocobros');

	}

	public function view($id){
		$data = array(
			'tipocobros' => $this->TipoCobros_model->getTipoCobro($id),
			// 'ciudades' => $this->Ciudades_model->getCiudades(),
		 );
			$this->load->view("tipocobros/view",$data);
	}
	...
	//este metodo retorna un valor verdadero
		if($this->form_validation->run() == TRUE){
			$data = array(
				...
				'id_tipocobro' 	=>$_POST['id_tipocobro'],
				'descripcion' 	=> strtoupper($_POST['descripcion']),
				// strtoupper$this->input->post("descripcion_tipoevento"),
				'estado' => "1"
			);
			if ($this->TipoCobros_model->save($data)) {
				$this->session->set_flashdata("success","Datos Guardados");
				redirect(base_url()."TipoCobros_controller", "refresh");
			}
			else{
	...
	public function edit($id){
		$data = array(
			...
			$this->load->view('tipocobros/edit', $data);
    }
	...
	public function update()
	{
		...
		$id_bd = $this->TipoCobros_model->getTipoCobro($id);
		...
		{	
			...
			$unique = '|is_unique[tipo_cobro.descripcion]';
		}
		...
		if($this->form_validation->run() == TRUE)
		{
			...
			if($this->TipoCobros_model->update($id,$data))
			{
